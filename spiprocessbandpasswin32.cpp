...
///////////////////////////////////////////////////////////////
// spiprocessbandpasswin32.cpp : Defines the entry point for the application.
//
//bandpass filter
//
...
///////////////////////////////////////////////////////////////
#include "stdafx.h"
#include "spiprocessbandpasswin32.h"
#include "FreeImage.h"
...
TCHAR szTitle[1024]={L"spiprocessbandpasswin32title"};					// The title bar text
TCHAR szWindowClass[1024]={L"spiprocessbandpasswin32class"};				// the main window class name
...
static int bandpassCallback( const void *inputBuffer, void *outputBuffer,
                         unsigned long framesPerBuffer,
                         const PaStreamCallbackTimeInfo* timeInfo,
                         PaStreamCallbackFlags statusFlags,
                         void *userData );
...
void CALLBACK StartGlobalProcess(UINT uTimerID, UINT uMsg, DWORD dwUser, DWORD dw1, DWORD dw2)
{
	...
	while( (global_duration_sec<0.0f) || ((nowstamp_ms-global_startstamp_ms)/1000.0f)<global_duration_sec )
	{
		...
		*/
		...
		//dynamically change filter parameter
		if(goingup==true) centerfreq_hz = centerfreq_hz + 100;
			else centerfreq_hz = centerfreq_hz - 100;

		if(centerfreq_hz>=1500) 
		{
			goingup=false;
		}
